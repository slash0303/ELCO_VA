...
class LogE:
    ...
    @staticmethod
    def p(log_name, progress, progress_max):
        tim = time.localtime(time.time())
        time_format = "%Y-%m-%d %H:%M:%S"
        Log_time = str(time.strftime(time_format, tim))
        WIDE = 20
        if progress_max != 1:
            progress_rate = int(progress/progress_max * WIDE)
        else:
            progress_rate = int(progress * WIDE)
        print(f"{Log_time} | {log_name} : [{progress_rate*'*' + (WIDE - progress_rate)*' '}]",end="\r")

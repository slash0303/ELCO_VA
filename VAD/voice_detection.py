...
def use_vad_based_recording(audio_stream_shm_name: str,
    ...
    while not transcript_loading_complete_flag.is_set():
        pass

    LogE.g(PROCESS_NAME, "voice detection started.")
    # Main loop
    while True:
        if not (trascript_processing_flag.is_set() or llm_response_generating_flag.is_set()):
            ...
            if recording_time.is_over() or (detection_state.name != "deactivate" and (speech_time.is_over() or pause_time.is_over())):
                ...
                if wrt == 1:
                    ...
                    # LogE.E(PROCESS_NAME, "npy was written")
                    # with open("./shm_vad.txt", "w", encoding="utf-8") as f:
                    #     f.write(str(speech_data_ndarr))
